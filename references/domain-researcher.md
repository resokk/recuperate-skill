...
- **Your file starts with a line naming who generated it and when.** Right after the H1 title: `_Generated by \`domain-researcher\` — <UTC timestamp, ISO 8601>._`. Get the timestamp via `date -u +%Y-%m-%dT%H:%M:%SZ` (or equivalent) when you start the run.
...
- **Report in the three-level hierarchy, always**: app-level tier (Backend / Frontend / DB) → functional category within that tier (infrastructure, data access layer, security, utils, business logic, mappers, engines, storage, simple controls, complex controls, or another category the code's own structure calls for) → individual found domain/concern (subdomain), one entry per concrete piece of the domain's implementation. Never fall back to flat, cross-cutting sections — a finding belongs to exactly one tier and one category, not a loose top-level list. Omit a tier with nothing found for it; never pad the structure with an empty section just to show the shape.
- **Each subdomain gets its own third-level heading (`###`) directly under its tier — never a fourth-level heading, and never a table row.** The tier is `##`, the subdomain is `###`; there is no separate heading level for the category in between. Category is a field *inside* the subdomain's own section (`**Category:** Business Logic`), not a heading that groups multiple subdomains under it. List subdomains within a tier ordered by category, so ones sharing a category still read together even without a heading grouping them. Never collapse a tier's subdomains into a table (one row per subdomain) — a table strips the space needed for the full detail every entry requires; each subdomain is a standalone `###` section with its own bullet fields, always.
...
2. **Classify each finding into the three-level hierarchy**:
   ...
   - **Functional category within that tier**: infrastructure, data access layer, security, utils, business logic, mappers, engines, storage, simple controls, complex controls, or another category the code's own structure actually calls for — pick the one the file's role fits, don't force a bad fit. Recorded as a field inside the subdomain's own section, not a heading of its own.
   - **Found domain/concern (subdomain)**: the specific, nameable piece of this domain's implementation living at that (tier, category) intersection (e.g. under Backend, category Business Logic: "invoice proration," "refund eligibility check") — this is the unit each detailed entry documents, as its own third-level heading directly under the tier.
...
_Generated by `domain-researcher` — 2026-09-05T14:32:00Z._
...
(more `###` subdomains as found, ordered by category so entries sharing one still read together — omit a category entirely from this tier if nothing was found for it, but never omit or table-ify a subdomain that was found)
...
(other subdomains as applicable, same structure)
...
Only emit the tiers (Backend/Frontend/DB) that actually have findings, and within each, only the subdomain (`###`) sections actually found — never force all three tiers to appear, never pad in a category with nothing found for it, and never represent a found subdomain as anything other than its own full `###` section.
