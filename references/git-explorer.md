...
- **Write your own text in English, regardless of the user's language or locale settings.** This applies to your own notes and any run-summary text. It never applies to a commit message or a Jira summary/description — those go into `messages.md`/`story.md`/`bug.md` verbatim, in whatever language they were actually written in, per the message-fidelity rule above.
- **Every one of the six files starts with a line naming who generated it and when.** Right after the H1 title: `_Generated by \`git-explorer\` — <UTC timestamp, ISO 8601>._`. Capture the timestamp once, in the extraction script (Workflow step 5), and reuse that exact value for every file this run produces, including the ones written later in the Jira/Confluence phase — don't re-query the time per file or per phase.
...
- **When a build does happen** (the skip check above found nothing already there), it's a **full rebuild every run.** All six output files (`messages.md`, `ticket.md`, `story.md`, `bug.md`, `story-links.md`, `bug-link.md`) are overwritten from a fresh `git log`/Jira fetch each run rather than patched incrementally — none of this output needs incremental freshness, and a full rebuild avoids stale or duplicated entries.
...
6. For each ticket ID found in step 5, fetch its Jira issue (summary, description, issue type) via an available MCP Jira tool, and also fetch its linked pages/remote links via the same MCP tool if it exposes that operation.
7. Route each successfully-fetched ticket into `story.md` (issue type `Story`) or `bug.md` (issue type `Bug`); write one section per ticket with its ID, summary, full description, and the Confluence links found from both the linked-pages lookup and a text scan of that description (merged, deduplicated).
8. Extract the Confluence links out of the finished `story.md` and `bug.md` into `story-links.md` and `bug-link.md` respectively — one deduplicated, sorted list of URLs per file.
...
# Captured once here and reused for every file this whole run produces,
# including story.md/bugs.md/story-links.md/bug-link.md written later
# in the Jira/Confluence phase -- never re-queried per file.
...
print(f"GENERATED_AT={GENERATED_AT}")  # reuse this exact value in story.md/bug.md/story-links.md/bug-link.md later
```
...
8. For each ticket ID from step 5, call the MCP tool's issue-lookup operation for that ticket key, requesting its summary, description, and issue type (or the closest fields it exposes); treat a not-found/error response as a skip rather than retrying indefinitely. If the MCP tool also exposes a linked-pages/remote-links operation for an issue, call that too for the same ticket.
9. For each successful lookup, read the issue's type field to route it (`Story` → story bucket, `Bug` → bug bucket, anything else → skipped-and-noted), extract plain text from the description if it came back as structured content, and build the ticket's Confluence link list from two sources: (a) any linked pages/remote links from step 8 whose URL host is `confluence.delta.sbrf.ru` or `confluence.sberbank.ru` (or, as a fallback, contains `/wiki/` in the path or `confluence` in the host), and (b) a scan of the summary + description text for URLs matching that same host check. Merge and deduplicate the two.
10. Write `story.md` and `bug.md`, each starting with `_Generated by \`git-explorer\` — <GENERATED_AT from step 5's script output>._`, then one section per routed ticket: ID, summary, full description, and its Confluence link list (or "None found.").
11. Grep the just-written `story.md` for Confluence URLs and write the deduplicated, sorted list to `story-links.md` (with the same generation line at the top); do the same for `bug.md` → `bug-link.md`.
...
**`.cache/recuperate/git/bug.md`** (same structure, one section per `Bug`-type ticket):
...
**`.cache/recuperate/git/story-links.md`** and **`.cache/recuperate/git/bug-link.md`** — the Confluence URLs extracted from `story.md`/`bug.md`, deduplicated and sorted, one per line:
