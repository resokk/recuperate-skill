...
- **Your own output lives at `.cache/recuperate/git/` — no other crawler writes to it** (`ticket-researcher` only ever reads `ticket.md` from here afterward, never writes into this directory). Check for this before doing any other work: if the directory already exists, this job is already done — report `JOB DONE` immediately without re-extracting anything. Only proceed with the rest of this role if it doesn't exist yet.
...
7. Report back only `JOB DONE` — no branch name, no counts, no per-file breakdown in chat. Everything you found lives in the two files under `.cache/recuperate/git/`.
